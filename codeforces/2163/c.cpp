...
int cheminHaut(int l, int r){
    ...
    if (prefMin[0].first<l || prefMax[0].first>r) return 0;
    int a=0;
    int b=n;
    while(b-a>1){
        int m=(b+a)/2ll;
...
int cheminBas(int l, int r){
    // find the largest index that is ok
    if (prefMin[0].second<l || prefMax[0].second>r) return 0;
    int a=0;
    int b=n;
    while(b-a>1){
        int m=(b+a)/2ll;
...
void solve(){
    ...
    int r=1ll;
    int tot=0;
    while (r<=2ll*n && cheminHaut(l,r)+cheminBas(l,r)<=n) r++;
    if (l<=min(tab1[0], tab2[0]) && r>=max(tab1[0], tab2[0])) tot+=(2ll*n-r+1ll);
    while(l<2ll*n){
        if (l>=r)r=l+1;
        l++;
        while (r<=2ll*n && cheminHaut(l,r)+cheminBas(l,r)<=n) r++;
